...
import com.knife.core.BlogConfig;
...
//	}
	public void addCategorySuper(){
		setAttr("count", Db.queryLong("select count(id) from kf_category_super"));
		render("addCategorySuper.html");
	}
	public void addCategorySub(){
		setAttr("pId", getPara("pId"));
		setAttr("count", Db.queryLong("select count(id) from kf_category_sub where pId=?",getPara("pId")));
	...
	public void editCategorySub() {
		...
		setAttr("count", Db.queryLong("select count(id) from kf_category_sub where pId=?",categorySub.getPcategory().getInt("id")));
	...
	public void loadCategorySub(){
		Set<CategorySub> categorySubList = new TreeSet<CategorySub>(CategorySub.dao.find("select * from kf_category_sub where pId = ?",getParaToInt("pId")));
